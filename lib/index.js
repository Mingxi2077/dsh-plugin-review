...
function ReviewSettingsPanel(props) {
  ...
  return React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 14, maxWidth: 620, color: tokenFg } },
    ...
    React.createElement('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 } },
      ...
      React.createElement('div', { style: fieldStyle },
        React.createElement('span', { style: fieldLabel }, '真实工具检查（评分背书）'),
        React.createElement('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 8 } },
          [['runTests', '运行测试'], ['runLint', '类型检查/Lint'], ['auditDeps', 'npm audit 依赖审计']].map(([k, label]) => {
            ...
            return React.createElement('label', {
              ...
              React.createElement('input', { type: 'checkbox', checked: on, onChange: () => setCfg({ ...cfg, [k]: !on }), style: { display: 'none' } }),
              label,
            )
          }),
        ),
        React.createElement('span', { style: { fontSize: 12, color: tokenSec } }, '测试/类型检查用项目内已装工具（不联网）；依赖审计需联网，默认关闭。'),
      ),
    ),
